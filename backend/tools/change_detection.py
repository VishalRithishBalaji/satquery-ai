...
from ..evidence.confidence import change_evidence_strength, compute_confidence
...
def change_detection_tool(state):
    ...
    confidence=compute_confidence(base=0.5,evidence_strength=change_evidence_strength(frac),input_quality=state.get('input_quality',1.0))
    return {'answer':summary,'confidence':confidence,'tool':'change_detection','model':'OpenCV change-map prototype','evidence':[{'type':'change_map','path':r['path'],'changed_fraction':frac,'regions':r['changed_regions']}]}
